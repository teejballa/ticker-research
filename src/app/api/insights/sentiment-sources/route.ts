...
//
// Phase 20-C-01: JSON endpoint backing the /insights/sentiment-sources
// dashboard. Returns the LATEST PerSourceIC row per (source × horizon) plus
// the second-latest row so the auto-down-weight signal (icir_20d < 0.3 for
// 2 consecutive windows) can be computed in-place.
...
export async function fetchSentimentSourcesPayload(): Promise<SentimentSourcesResponse> {
  ...
  for (const source_id of SOURCES) {
    ...
    for (const horizon of HORIZONS) {
      ...
      if (rows.length === 0) {
        // No row → tile is null (cold start).
