# Phase 28 — Real Sentiment Analysis & Report-Generation Excellence
...
These are the non-negotiables — every plan in Phase 28 must demonstrate adherence in its own PLAN.md sign-off.

### S1 — No hand-picked parameters
Every threshold, half-life, prior strength, weight cap, or class boundary must be either (a) cited from a peer-reviewed source whose ticker class / time period matches Cipher's, OR (b) calibrated on labeled production data via the documented procedure in 28-Z-05 (LLM-as-judge / human eval harness). Hand-picking is allowed only for explicit experiment defaults that the calibration step then overrides.

### S2 — Point-in-time (PIT) discipline
Every sentiment feature must be queryable by `fetched_at` (when *we* recorded it), never by `published_at` (which upstream sources may revise/edit). Persisted snapshots are immutable. Any backfill that recomputes a feature creates a new column or model_version, never overwrites. Lookahead-bias regression test (28-Z-07) fails the build if any feature joins on `published_at` for backtest queries.
...
Every classifier, ensemble, or composite signal that ships in Phase 28 produces a Mitchell-2019-style model card (`MODEL-CARD-{component}.md`) covering: training data + size, evaluation metrics with CIs, intended use, out-of-distribution behavior, ethical considerations, known failure modes, retrain cadence. No model goes to `on` without one.

### S5 — Pinned model + prompt versions
Every external model invocation pins SHA / revision / endpoint version. Every Gemini prompt is a versioned artifact in a registry (28-Z-04) with golden-file regression on every change.

### S6 — Telemetry on every external call
Every external provider call surfaces latency p50/p95/p99, error rate, cost per request, cache hit rate, and fallback rate to a `/insights` "Sentiment Health" tab (28-Z-03). Cost per ticker per request is computed and budget-alerting at 1.5× rolling baseline.
...
Every report-generation change is regression-tested against a fixed set of "golden tickers" spanning {large-cap-equity, mid-cap-equity, micro-cap-equity, ETF, SPAC, ADR, recently-listed, low-coverage} (28-D-04). A change that fails any golden ticker auto-blocks merge.

### S10 — Regulatory hygiene
Every report contains required disclaimers (no personalized investment advice, data-as-of timestamps, source attribution). 28-D-05 is the audit. **Phase 28 does NOT publish public-per-user calibration data** — that lives behind Phase 27's legal-counsel gate.
...
4. **Source-tier weights data-driven** from 28-C-01 IC measurements (no hand-curated entries shipping).
...
10. **Numeric-grounding test green** on all 8 golden tickers (28-D-01).
11. **Citation-coverage ≥ 80%** per report on all 8 golden tickers (28-D-02).
12. **Model cards exist** for every shipped sentiment artifact (Mitchell 2019 format).
13. **Lookahead-bias test green** (28-Z-07).
...
|---|---|
| 28-Z-01 | Prisma schema migration + db push (additive, non-blocking). |
| 28-Z-03 | Optional: provision OTel collector in Vercel for richer trace export (deferrable — internal dashboard works without it). |
| 28-B-02 | Provision `ProsusAI/finbert` HF Inference Endpoint at pinned SHA on $0.033/hr CPU instance; set `HF_FINBERT_ENDPOINT` + `HF_INFERENCE_TOKEN` in Vercel prod. |
| 28-A-02, 28-C-01 | Ensure backfill compute capacity for nightly cron (current crons run in <5 min; this phase adds ~3-5 min). |
| 28-D-04 | Curate the 8 golden-ticker snapshots once (one-time human curation step; ~2 hours). |
| 28-C-04 | Optionally license NASDAQ surveillance-alert feed for true labeled P&D ground truth (defer — synthetic-injection eval is sufficient for ship). |
| Wave-D ship | Optional legal review of disclaimer template before public ship (consistent with Phase 27 entry gate; this phase does NOT require it because nothing publishes outside the existing auth-gated UI). |

---

## Out of scope (deferred to later phases)

- **Multimodal price + sentiment fusion model** — Phase 22 (Composite Signal Synthesis).
...
- **Public per-report calibration trail** — Phase 27 (legal-counsel gate).
- **Deep historical sentiment backfill** — Phase 25 (Historical Backfill).
- **Cross-asset sentiment correlation features** (e.g., sector-ETF sentiment as a covariate) — backlog candidate; defer until Phase 22 composite scaffolding lands.
...
|---|---|---|
| T-28-001 | Bot/coordination floods skew sentiment | 28-C-03 + 28-C-04 |
| T-28-002 | Lookahead bias in backtest | 28-Z-01 + 28-Z-07 (PIT discipline) |
| T-28-003 | Vendor source rot (StockTwits API change, ApeWisdom shutdown) | 28-Z-03 telemetry alerts on error rate spike; 28-B-06 lexicon fallback |
| T-28-004 | Classifier upgrade silently changes scoring | 28-Z-01 model_version columns; 28-Z-04 prompt registry; 28-B-03 ECE re-fit |
| T-28-005 | Cost runaway (FinBERT endpoint or Gemini per-doc) | 28-Z-03 cost-budget alert at 1.5× rolling baseline; FinBERT only fires when message_volume > 50 |
| T-28-006 | Hallucinated numbers in report | 28-D-01 numeric-grounding test |
| T-28-007 | Hallucinated citations in report | 28-D-02 + 19-C-07 (existing) |
| T-28-008 | Crowded consensus mistaken for thesis | 28-A-01 + 28-C-04 |
| T-28-009 | Aspect tagging inconsistent across runs | 28-B-05 Cohen's kappa target |
| T-28-010 | Calibration drifts with regime change | 28-B-03 monthly ECE refit cron |
| T-28-011 | Bot-filter false positives suppress real signal | 28-C-03 ≤5% FP gate; spot-check audit |
| T-28-012 | Disclaimer rotted / missing on report | 28-D-05 audit script CI gate |
| T-28-013 | Fairness gap (e.g., poor calibration on micro-caps or non-US) | 28-C-06 per-segment audit |
| T-28-014 | Prompt change breaks downstream reports | 28-Z-04 golden-file regression |
| T-28-015 | Per-source IC degraded → bad weight | 28-C-01 auto-down-weight on ICIR < 0.3 |

---

## MVP slice (ship first if scope-cut needed)

If we have to ship narrower: **Wave Z + 28-A-01 (crowded_consensus flag) + 28-A-03 (time decay) + 28-B-01 (Gemini per-doc) + 28-D-01 (numeric grounding) + 28-D-04 (golden tickers).** That's 12 plans and delivers the GME-100% fix + per-document NLP + report-quality regression, leaving the rigor (per-source IC, bot detection, fairness audit) for a Phase 28.5 follow-on.
