...
7. **PIT Gate**: 20-Z-07 (SHIPPED — see .planning/phases/20-real-sentiment-analysis/20-Z-07-PLAN.md) instruments the production query path and asserts no SQL references `published_at` for backtest joins. This plan ships the schema columns; 20-Z-07 ships the runtime defense.
...
| T-20-Z-01-03 | Tampering / lookahead bias | Future queries joining on `published_at` for backtests inflate IC results | mitigate | `// PIT-INVARIANT` grep marker on the `fetched_at` column. `published_at` is nullable and documented "informational only" in-line. 20-Z-07 (SHIPPED — see .planning/phases/20-real-sentiment-analysis/20-Z-07-PLAN.md) ships the regression test that fails the build on any SQL/ORM call joining on `published_at` for backtest paths. **Maps to phase catalog T-28-002.** **Severity: HIGH** — mitigation is REQUIRED in this plan (cannot defer to 20-Z-07 because the schema must be PIT-shaped from day 1). |
