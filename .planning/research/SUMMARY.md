...
v2.0 is **buildable in pure TypeScript on the existing Vercel + Neon stack** with **3 new npm packages** (`jstat`, `ml-matrix`, `posthog-node`). All 10 phases (P18–P29) are strictly additive — no v1.0 module is rewritten. The architectural foundation laid in v1.0 (`learning.ts` is "pure functions, no DB"; `engine-context.ts` is the single trust boundary for authoritative numerics) is the right shape for these enhancements.
...
| `posthog-node` | latest | Lightweight metric collection for Phase 28 dashboard (alternative: just write to `LearningEvent` table) |
...
1. **P27 (historical backfill) must come before P23 (lift gating).** With only 87 PriceOutcome rows today, walk-forward CV for lift estimation is too noisy. Backfill bootstraps the N needed for statistically meaningful lift gating.

2. **P22 (regime feature) is the highest-risk migration** — it changes the `LearnedPattern` composite unique key. Must ship EARLY (when fewer dependent rows exist) with a 2-step migration: add column with `DEFAULT 'ALL'`, soak, then add new constraint.

3. **P18 (time-decay) is the keystone** — it defines `effective_sample_size` (ESS), which P19, P23, and P28 all consume. Must ship first.

4. **Pitfall 1 (multiple comparisons / lake of cells) is the dominant risk.** With ~1,728 cells once P22 adds regime dimension, expect ~86 spuriously ACTIVE cells under the null at α=0.05 if no FDR correction is applied. **This is exactly the failure mode v1.0 ACTIVE cells exhibit (0% Brier-lift).** Phase 23 must include FDR correction (Benjamini-Yekutieli).

5. **Pitfall 2 (temporal CV leakage) is the second-biggest risk** because horizons (3/7/14/30/60/90d) overlap on the same SentimentSnapshot rows. Purged K-Fold + Embargo (López de Prado) is the standard defense, must apply to both P23 promotion and P27 backfill.

6. **Phase 29 (public calibration trail) needs legal sign-off pre-launch.** SEC/FINRA compliance risk for public ML predictions about specific securities is the only pitfall whose recovery cost is "may threaten the project itself."

## Reconciled Phase Order (P18 → P29)

Resolves between Architecture's "P18→P22→P19→P23" and Features' "P27 before P23":
...
| **19** | Hierarchical priors / partial pooling | Needs P18's ESS + P22's regime dimension as a pooling axis |
| **25** | Historical backfill | Bootstraps N needed for P23's lift gating to be meaningful |
| **21** | Lift-gated cell promotion | Needs P27's data + FDR correction + Purged CV |
| **22** | Composite signal synthesis | First user-visible UX win — composite headline number with CI |
| **23** | Counterfactual reasoning in prompt | Needs P24 composite to counterfactual against |
| **24** | Adaptive watchlist (Thompson sampling bandit) | Parallel-eligible with P24/P25, no shared files |
| **26** | Live performance dashboard | Needs everything else to have produced numbers worth surfacing |
| **27** | Public per-report calibration trail | Last — needs legal sign-off + all metrics from P28 |

**Parallelization opportunities** identified by file-level disjointness: P24 ↔ P26, P25 ↔ P28.

## Requirements Themes (drives REQUIREMENTS.md)

### CORE-ML (Group A) — 4 capability areas
Drift defense (P18), hierarchical pooling (P19), regime awareness (P22), lift-gated promotion (P23). Industry-standard references: Stan partial-pooling case studies, ADWIN/Page-Hinkley for drift, López de Prado for purged CV, Benjamini-Yekutieli for FDR.

### REASON (Group B) — 2 capability areas
Composite signal (P24), counterfactual reasoning (P25). Industry-standard references: scikit-learn isotonic calibration, Brunswik lens model, EU AI Act 2026 explainability standards.

### COVERAGE (Group C) — 2 capability areas
Adaptive watchlist (P26), historical backfill (P27). Industry-standard references: Russo & Van Roy Thompson-sampling tutorial, point-in-time data per CFA curriculum.

### DEMO (Group D) — 2 capability areas
Performance dashboard (P28), public calibration trail (P29). Industry-standard references: Mitchell et al. 2019 model cards, MLflow/W&B dashboards, SEC Predictive Analytics Rule 2023-140 compliance.
...
- **Phase 29 entry gate is "legal counsel engaged"**, not "feature ready to ship"
...
3. **Regime feature integrated** into LearnedPattern key with deterministic regime labels for all backfilled and live snapshots (P22)
4. **≥1 cell with FDR-corrected, Purged-CV out-of-sample Brier-lift > 5%** vs null model (P23)
5. **Composite signal block in every report** showing calibrated headline probability + CI + per-class breakdown (P24)
6. **Counterfactual deltas in every report** explaining how each signal class moved the thesis (P25)
7. **Adaptive watchlist live** with Thompson-sampling-driven undersampled-cell prioritization (P26)
8. **Backfill universe expanded** to ≥100 tickers × 5 years with point-in-time correctness (P27)
9. **Performance dashboard live at /insights** with daily learning feed + drift alerts (P28)
10. **Per-report calibration trail published** with legal sign-off + aggregate-only public metrics (P29)
