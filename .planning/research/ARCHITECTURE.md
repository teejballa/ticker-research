...
│  │ + Calibration    │  │ + perf tab (P28)│  │ ★ NEW (P29)            │  │
...
│  │   ├─ ★ buildCompositeBlock (P24)        ← composite signal prob   │  │
│  │   └─ ★ buildCounterfactualBlock (P25)   ← "if signal absent..."   │  │
│  └────────────────────────────┬────────────────────────────────────┬─┘  │
│                               │                                    │     │
├───────────────────────────────┼────────────────────────────────────┼─────┤
...
│  │   ├─ ★ resolveRegimeKey (P22)            ← +regime to cell key │ │     │
│  │   ├─ ★ composeMultiCellSignal (P24)      ← 4-class → composite │ │     │
│  │   └─ ★ counterfactualDeltas (P25)        ← per-class ablation  │ │     │
│  └────────────────────────────┬──────────────────────────────────┘ │     │
│                               │                                    │     │
├───────────────────────────────┼────────────────────────────────────┼─────┤
...
│  │ (existing)      │  │  Detector(P22)│  │   (P23)         │      │     │
│  └─────────────────┘  └───────────────┘  └─────────────────┘      │     │
│  ┌─────────────────┐  ┌───────────────┐  ┌─────────────────┐      │     │
│  │ adversarialNull │  │ ★ temporalCV  │  │ ★ thompsonPick  │      │     │
│  │ Brier (existing)│  │  (P23)        │  │  (P26)          │      │     │
│  └─────────────────┘  └───────────────┘  └─────────────────┘      │     │
│                                                                    │     │
├──────────────────────────────────────────────────────────────────┬─┴─────┤
...
│  │  (P26)          │  │  from regime-  │  │ ★ regime tag (P22) ││       │
│  │                 │  │  tagged rows)  │  │ ★ lift gate (P23)  ││       │
│  └─────────────────┘  └────────────────┘  └────────────────────┘│       │
│  ┌─────────────────┐  ┌────────────────┐                        │       │
│  │ ★ regime-detect │  │ ★ backfill     │                        │       │
│  │  (P22, daily)   │  │  (P27, manual  │                        │       │
│  │                 │  │  + nightly     │                        │       │
│  │                 │  │  resume)       │                        │       │
│  └─────────────────┘  └────────────────┘                        │       │
├──────────────────────────────────────────────────────────────────┼───────┤
...
│  │ + regime col(P22)│  │  (P22)           │ │  Snapshot (P28)│         │
│  │ + parent_key(P19)│  │  daily row       │ │  daily KPIs    │         │
│  │ + ess col (P18)  │  └──────────────────┘ └────────────────┘         │
│  └──────────────────┘  ┌──────────────────┐ ┌────────────────┐         │
│  ┌──────────────────┐  │ ★ BackfillCursor │ │ ★ TrailEvent   │         │
│  │ LearningEvent    │  │  (P27)           │ │  (P29, optional│         │
...
| **Build order** | Foundational. Should ship FIRST after a single combined schema migration. P19 hierarchical pooling needs `effective_sample_size` to weight parent vs cell mixing. P23 lift-gate uses `effective_sample_size` not raw `sample_size` as the threshold gate. |
...
| **Build order** | After P18 (uses `effective_sample_size` to weight pooling — small-n cells pull harder toward parent). Before P24 (composite signal reads pooled posteriors when available). |
...
| **Backwards compat** | Reads remain on `alpha/beta` until P24 explicitly switches the calibration block to `pooled_alpha/pooled_beta`. |
...
### Phase 22 — Market-Regime Feature
...
### Phase 23 — Lift-Gated Cell Promotion + Out-of-Sample Temporal CV
...
| **Build order** | After P18 (uses ESS in promotion gate). After P19 (CV runs against pooled posteriors). After P22 (CV is per-regime — must respect new key). Before P24 (composite signal weights cells by `lift_pct`, not just status). |
...
### Phase 24 — Composite Signal Synthesis in the Prompt
...
| **Schema impact** | None to LearnedPattern. Optional new column `Report.composite_signal Json?` to persist the snapshot for the trail page (P29). |
| **Build order** | After P19 (uses pooled posteriors when available) and P23 (uses `lift_pct` as the inverse-variance weight). Strict prerequisite for the "headline number" UX promise. |
...
### Phase 25 — Counterfactual Reasoning in Reports
...
| **Build order** | Direct dependency on P24 (composite must exist to compute counterfactuals). Otherwise independent. |
...
### Phase 26 — Adaptive Watchlist via Bandit
...
| **Build order** | After P18 (bandit reward function is `effective_sample_size` of underrepresented cells). After P22 (bandit must consider regime — can't recommend tickers for cells in a regime that's already saturated this cycle). Independent of P19, P23, P24, P25. **Parallelizable with P24+P25.** |
...
### Phase 27 — Historical Backfill from Price Data
...
| **Build order** | Should ship AFTER P18 + P22 + P23 — backfill needs to assign decay weight (P18 — historical observations should have decayed weight from day 1), regime tag (P22 — must look up the regime AT the historical date), and lift gate (P23 — must temporal-CV across the backfilled history). Otherwise we backfill 5y of data into v1.0-shape cells and have to recompute everything. |
...
### Phase 28 — Live Engine Performance Dashboard
...
| **Build order** | After P23 (KPIs include lift). Otherwise independent of all v2.0 work — can ship in parallel with P24/P25/P26. **Parallelizable.** |
...
### Phase 29 — Public Calibration Trail
...
| **Build order** | After P24 (composite signal is a primary item in the trail). After P28 (shares perf-snapshot data). LAST PHASE — depends on everything to look impressive. |
| **Disruption risk** | NIL. New page, new route. Existing report rendering untouched. |
| **Backwards compat** | Reports older than P24 simply show "composite signal not available — this report predates v2.0". |
...
├── engine-context.ts              # extended in P19, P22, P24, P25
├── gemini-analysis.ts             # extended in P24, P25 (new prompt blocks)
├── data/
│   └── ticker-watchlist.ts        # extended in P26
└── learning-v2/
    ├── decay/                     # P18
...
├── api/
│   ├── cron/
│   │   ├── learn/route.ts            # extended P18, P19, P22, P23
│   │   ├── sentiment-scan/route.ts   # extended P26
│   │   ├── price-followup/route.ts   # UNCHANGED (no v2.0 dependencies)
│   │   ├── regime-detect/            # ★ NEW P22
│   │   │   └── route.ts
│   │   └── backfill-resume/          # ★ NEW P27
│   │       └── route.ts
│   └── insights/
│       └── performance/              # ★ NEW P28
│           └── route.ts
├── insights/
│   ├── page.tsx                       # extended P22 (regime banner), P28 (perf tab)
│   └── components/                    # new tabs
└── report/
    └── [id]/
        └── trail/                     # ★ NEW P29
...
**When:** Any capability that modifies an existing module's behaviour (P18, P19, P22, P23, P24, P26). New surfaces (P28, P29) don't need flags.
...
**When:** All v2.0 schema work. Especially relevant for P22 (regime in composite key) and P23 (status replacement).
...
await applyLiftGatePass();                     // P23 — NEW (overwrites status)
await persistLogisticEpoch(state, ...);        // existing
await persistEnginePerfSnapshot();             // P28 — NEW
...
**When:** All v2.0 prompt extensions (P24, P25) MUST follow this — composite signal numbers, counterfactual deltas come from `engine-context.ts`; LLM provides only `composite_narrative`, `counterfactual_narrative` strings.
...
```
06:00 UTC  /api/cron/regime-detect       (P22)
...
07:30 UTC  /api/cron/learn               (extended)
              ...
              ├─ applyLiftGatePass (P23) → overwrites status column
              ├─ persistLogisticEpoch    → unchanged
              ├─ persistEnginePerfSnap   → P28 KPIs
              └─ writeCycleSummary       → unchanged
08:00 UTC  /api/cron/sentiment-scan      (every 3d, extended)
              ├─ thompsonPickTickers     → P26 selects rotation slots
              ├─ for each ticker:
              │     ├─ fetch market data + community + technical + insider + institutional
              │     ├─ stamp regime_at_scan (P22)
              │     └─ write SentimentSnapshot
02:00 UTC  /api/cron/backfill-resume     (P27, nightly chunk)
```
...
POST /api/analysis/[ticker]
    → getEngineContextForTicker(ticker, asOf)
        ├─ resolveRegime(asOf)            → P22 — pin regime for THIS report
        ...
        ├─ composeMultiCellSignal         → P24 — inverse-variance weighted composite
        ├─ counterfactualDeltas           → P25 — per-class ablation deltas
    ...
    → buildSystemPrompt(ctx)
        ...
        + buildCompositeBlock              (P24 — NEW)
        + buildCounterfactualBlock         (P25 — NEW)
    → runGeminiAnalysis
    ...
    → SSE → /research/[ticker]
        ...
        + CompositeBadge (P24)
        + CounterfactualSection (P25)
        + "View Calibration Trail" link  (P29)
```
...
```
        ...
        Phase 19 ── pooling  │
            │     ↘          │
        Phase 23 ── lift-gate (CV) ──┐
            │     ↘                  │
        Phase 24 ── composite ──┐    │
            │                   │    │
            ├──────────────┐    │    │
            │              │    │    │
        Phase 25 ── CF   Phase 26 ── bandit (parallelizable from here)
            │              │    │    │
            │           Phase 28 ── perf dashboard (parallelizable)
            │              │    │    │
            │           Phase 27 ── backfill (uses 18+20+21)
            │              │    │    │
            ╰──────────────┴────┴────┴── Phase 29 ── trail (LAST — needs everything)
```
...
|---|---|---|---|
| **1** | **P18** | Time-decayed Bayesian updates | Foundational: provides `effective_sample_size` used by P19, P23, P26. Single-cell math, easy to ship and verify. Visible value: drift defense activates immediately; older observations decay; reports start citing ESS. |
| **2** | **P22** | Market-regime feature | Big schema change — do it EARLY when v2.0 has fewer dependent rows. Adds the regime dimension to the cell key, which P19, P23, P24 all need to respect. Visible value: "current regime" banner ships day one. |
| **3** | **P19** | Hierarchical priors / pooling | Needs P18 (ESS) and P22 (regime in key). Now sparse cells learn faster, which makes EVERY later phase look better. |
| **4** | **P23** | Lift-gated promotion + temporal CV | Needs P18, P19, P22. After this lands, the ACTIVE label MEANS something — primary v1.1 DoD criterion. |
| **5** | **P24** | Composite signal in prompt | First user-visible UX win after the math foundation: every report now has a single headline calibrated probability with credible interval. Requires P19, P23. |
| **6** | **P25** | Counterfactual reasoning | Direct dependency on P24. Cheap incremental win. |
| **7** | **P28** | Live performance dashboard | Independent of P24-P27 (only depends on P23). **Can ship in parallel with P24/P25.** Needed before public-facing P29 to demonstrate the engine is provably better. |
| **8** | **P26** | Adaptive watchlist (bandit) | Independent of P24/P25. **Can ship in parallel with P28.** Needs only P18 + P22. Ships value: cells fill faster, lift improves. |
| **9** | **P27** | Historical backfill | Needs P18 + P22 + P23 ALL settled — backfilling into a still-shifting cell shape is wasted work. Big-bang capability that takes the Brier-lift goal across the line. |
| **10** | **P29** | Public calibration trail | LAST. Needs everything to look polished and impressive. Pure presentation — no math risk. |

### Parallelization Opportunities

- **P24 ‖ P28** — composite signal (engine prompt) and perf dashboard (insights tab) touch different files. Can split between two contributors.
- **P26 ‖ P27 ‖ P29** — once P23 is landed, watchlist work, backfill work, and trail page work touch disjoint files (`ticker-watchlist.ts`, `scripts/backfill-*.ts`, `app/report/[id]/trail/`). Three-way parallel sprint.
- **P18 + P22 schema migration** — combine into ONE Prisma migration to avoid two separate downtime windows. The other schema additions (P19, P23, P28) can each be their own small migration.
...
|---|---|---|
| **v2.0-foundation** | `MarketRegime` table; `LearnedPattern` columns: `regime`, `effective_sample_size`, `decay_half_life_days`; `SentimentSnapshot.regime_at_scan`; new composite unique key; `LearningEvent.delta` JSON gains `decayed_weight` | P18 + P22 |
| **v2.0-pooling-cv** | `LearnedPattern` columns: `pooled_alpha`, `pooled_beta`, `parent_alpha`, `parent_beta`, `pooling_weight`, `brier_oos`, `brier_null_oos`, `lift_pct`, `cv_folds_used` | P19 + P23 |
| **v2.0-coverage** | `EnginePerfSnapshot` table; `WatchlistDecision` table; `BackfillCursor` table; `is_backfill` flags on `SentimentSnapshot` and `PriceOutcome`; `Report.composite_signal`, `Report.public_trail_token` | P24–P29 |
...
| P22 regime | **HIGH** (composite key) | MED (new cron) | MED (banner) | NIL | NIL | **MEDIUM-HIGH** |
| P23 lift-gate | LOW (additive cols) | LOW (replaces status) | MED (demotions visible) | NIL | NIL | **LOW-MED** |
| P24 composite | LOW | NIL | MED (headline badge) | MED (new block) | LOW (new field) | **MEDIUM** |
| P25 counterfactual | NIL | NIL | LOW (subsection) | LOW (new block) | LOW (new field) | **LOW** |
| P26 bandit | LOW (audit table) | LOW (modify scan) | LOW (diagnostics) | NIL | NIL | **LOW** |
| P27 backfill | LOW (flag cols) | MED (new cron + script) | LOW (status widget) | NIL | NIL | **MEDIUM** |
| P28 perf | LOW (1 new table) | LOW (1 new write per cycle) | MED (new tab) | NIL | NIL | **LOW** |
| P29 trail | LOW (1 nullable col) | NIL | MED (new page) | NIL | NIL | **LOW** |

**Highest-risk phase:** P22 because of the composite-key change. Mitigation: TWO-step migration (add column with default, then add new constraint) and a one-week soak in production with `regime='ALL'` for everyone before flipping the regime detector to write real labels.
...
**Instead:** New prompt blocks (P24, P25) follow the v1.0 pattern: numerics overwritten post-generation, LLM contributes only narrative strings.

### Anti-Pattern E — Backfilling before the cell shape is final

**What:** Running P27 backfill before P22 (regime) and P23 (lift-gate) settle.
**Why bad:** 4M backfilled rows into a key shape that's about to change = expensive recompute + risk of corruption.
**Instead:** P27 is the LAST math phase. Once it runs, the cell shape is locked.

---

## Scalability Considerations

| Concern | At v2.0 launch | After 6 mo of P27 backfill | At 1y |
|---|---|---|---|
...
| `learn` cron runtime | ~30s | ~90s (P19 + P23 added) | ~150s |
...
**Bottleneck risk:** P27 backfill's recompute pass scales as O(cells × events). At 4M events × 6,000 cells, the inner `recomputeOneCell` query budget becomes the limit. Mitigation: index `LearningEvent(signal_class, pattern_key, cap_class, horizon_days, regime)`; cap per-cell `take: 500` is already in v1.0.
...
- `/Users/tj/Desktop/Cipher/vercel.json` (cron schedule — adds for P22, P27)
- `/Users/tj/Desktop/Cipher/src/app/api/cron/learn/route.ts` (5-pass cron structure — insertion points for P18, P19, P23, P28)
- `/Users/tj/Desktop/Cipher/src/app/api/cron/sentiment-scan/route.ts` (watchlist call site — P26 hook)
- `/Users/tj/Desktop/Cipher/src/app/api/cron/price-followup/route.ts` (unchanged in v2.0 — confirmed no dependency)

**Confidence:** HIGH — every integration point named is traced to a real line in the live v1.0 codebase. Schema deltas are concrete and additive. Phase order respects observed code dependencies (e.g. P19 needs `effective_sample_size` which is a P18 column; P24 reads `pooled_alpha` which is P19; P27 backfills into shape locked by P22+P23).
