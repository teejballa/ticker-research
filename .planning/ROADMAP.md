...
- [ ] **Phase 18: Time-Decayed Bayesian Updates + ESS** — keystone phase. Adds `effective_sample_size` to LearnedPattern via exponential decay; Page-Hinkley drift detector; per-class λ tuning. Requirements: CORE-ML-01..05.
  ...
  - Plans:
    - [ ] 18-00-PLAN.md — Wave 0: 10 test stub files scaffolded before any implementation (TDD red→green setup)
    - [ ] 18-01-PLAN.md — Wave 1: decay/ESS/Page-Hinkley/confirmedDrift pure functions + STATUS_VALUES const in src/lib/learning.ts
    - [ ] 18-02-PLAN.md — Wave 1: src/lib/cv.ts purgedKFold (Purged K-Fold + Embargo CV per López de Prado)
    - [ ] 18-03-PLAN.md — Wave 1: additive Prisma schema migration (effective_sample_size, n_trials_attempted) + [BLOCKING] db push
