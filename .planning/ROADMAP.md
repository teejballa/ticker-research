...
- [ ] **Phase 27: Historical Backfill** — bootstrap N for lift gating: ≥100 tickers × ≥5 years of technical signals; point-in-time discipline; single feature-extraction code path. Requirements: COVERAGE-06..10.
...
- [ ] **Phase 29: Public Per-Report Calibration Trail + Model Card** — public-readable trail page; aggregate-only public stats; Mitchell 2019 model card. **Entry gate: legal counsel engaged.** Requirements: DEMO-07..11.
...
- [ ] **Phase 30.1 (INSERTED): Free Community-Scan Migration** — replace Firecrawl entirely with a free alternative for community-sentiment scraping. Operator hit Firecrawl's free tier on 2026-05-14 and chose migration over rotation. Scope: design the replacement architecture covering Reddit (currently 5 subreddit searches per ticker in `lightweight-community-scan.ts`) AND broader free sentiment sources (other subreddits, news/forum sites — operator wants more coverage than the current Reddit-only footprint). Likely Reddit replacement: official OAuth API (script-type app, free 100 QPM, structured JSON). Twitter/X scraping is essentially dead for free in 2026 — out of scope unless retriggered. Until 30.1 ships, Firecrawl provider remains in BREACH on Phase-30 Done-gate 1 (error_rate > 10%) — expected, documented in `firecrawl-rotation-log.md`. **Depends on:** Phase 30 (provides the breaker-wrapped adapter slot that 30.1's replacement plugs into).

### Parallelization Opportunities

After Phase 24 ships, Phases 25 / 26 / 28 share no files and can be planned in parallel. Phase 29 depends on Phase 28's metrics. Phase 20 (sentiment overhaul) and Phase 21 (sector-relative outcome labels) are independent of Phases 22-29 and run first — front-loaded after the 2026-05-10 reorder because the user-visible sentiment failure (100% bullish on meme stocks) is shipping in production today, and the SPY-vs-sector benchmark question affects every downstream phase that consumes the learned posteriors.
