...
- [ ] **Phase 28: Real Sentiment Analysis** — bring sentiment layer to industry baseline (RavenPack / MarketPsych parity on per-document NLP, source-tier weighting, time decay, dispersion/crowding flags, volume baselining, bot filters, per-source IC calibration). Triggered by post-19 finding that StockTwits raw bull% reads as 100% on meme stocks (GME) — single-source extreme treated as thesis confirmation when it's actually an echo-chamber crowding signal. 13 plans across 3 waves: Wave A (5 quick wins, no infra dep) ships dispersion+crowding flag, volume baselining, exponential time decay, author-diversity ratio, cross-platform agreement signal; Wave B (5 plans, requires HF endpoint OR Gemini per-message) adds per-document classification, FinBERT, temperature-scaled calibration, source-tier table, per-aspect headline numbers; Wave C (5 plans) closes calibration loop with per-input-source ICIR tracking + Brier decomposition, Cresci-2019 bot filter, Nam/Yang-2023 pump-and-dump cluster detection, sentiment×momentum×volume joint feature. **Context doc:** `.planning/phases/28-real-sentiment-analysis/CONTEXT.md` (research synthesis from 4 parallel agent runs covering RavenPack, MarketPsych, Loughran-McDonald, Tetlock, FinBERT, Cookson/Engelberg echo chambers, Cresci StockTwits bots, López de Prado purged CV).
